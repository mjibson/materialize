...
mod tests {
    ...
    use repr::{RowArena, ScalarType};

    use crate::{Format, Type, Value};

    #[test]
    fn encode_round_trip() {
        // Test that datums can be round-trip encoded and decoded.

        let datums = repr::tests::interesting_datums();
        let formats = vec![Format::Text, Format::Binary];

        let mut buf = BytesMut::new();
        let arena = RowArena::new();
        for (scalar, datum) in datums {
            for format in &formats {
                let value = Value::from_datum(datum, &scalar).expect("not null");
                let ty = Type::from(&scalar);
                println!("ENCODE TEST {}, {}, {:?}", scalar, datum, format);
                buf.clear();
                value.encode(&ty, *format, &mut buf).expect(&format!(
                    "can encode {:?} ({:?}) as {:?}",
                    value, ty, format
                ));
                let decoded_value = Value::decode(*format, &ty, &buf).expect(&format!(
                    "can decode {:?} ({:?}) as {:?}",
                    value, ty, format
                ));
                let orig_encoding = buf.split();
                decoded_value
                    .encode(&ty, *format, &mut buf)
                    .expect(&format!(
                        "can encode {:?} ({:?}) as {:?}",
                        decoded_value, ty, format
                    ));
                let roundtrip_encoding = buf.split();
                let (decoded_datum, decoded_scalar) = decoded_value.into_datum(&arena, &ty);

                // Decimals don't roundtrip due to scale changes so skip them.
                if let ScalarType::Decimal(_, _) = scalar {
                    continue;
                }

                // Verify that the decoded datum encodes identically to the original datum.
                assert_eq!(
                    orig_encoding, roundtrip_encoding,
                    "orig: {:?}, roundtrip: {:?}",
                    datum, decoded_datum
                );
                assert_eq!(scalar, decoded_scalar, "{:?}, {:?}", datum, decoded_datum);

                // Verify that the original datum is equal to the encoded -> decoded datum.
                assert_eq!(datum, decoded_datum);
            }
        }
    }

    #[test]
    fn pgwire_pgtest() -> Result<(), Box<dyn std::error::Error>> {
        // Test that datums encode identically to Postgres. This test uses datadriven
        // which will rewrite its data if env var REWRITE=1. We will use the same env
        // var here to also create a pgtest file generated from interesting_datums.

        let datums = repr::tests::interesting_datums();
        let formats = vec![Format::Text, Format::Binary];
        let path: PathBuf = ["tests", "testdata", "pgwire.pt"].iter().collect();
        if env::var_os("REWRITE").unwrap_or("".into()) == "1" {
            let mut file = File::create(path).unwrap();
            writeln!(&mut file, "# DO NOT EDIT.")?;
            writeln!(&mut file, "# AUTO GENERATED BY pgwire_pgtest.")?;
            writeln!(&mut file, "")?;
            for (scalar, datum) in &datums {
                for format in &formats {
                    ...
                    writeln!(&mut file, "send")?;
                    writeln!(&mut file, r#"Parse {{"query": "SELECT $1::{}"}}"#, scalar)?;
                    writeln!(
                        &mut file,
                        r#"Bind {{"result_formats": [{}], "values": [{}]}}"#,
                        format, datum
                    )?;
                    writeln!(&mut file, "----")?;
                    writeln!(&mut file, "")?;
                    writeln!(&mut file, "until")?;
                    writeln!(&mut file, "ReadyForQuery")?;
                    writeln!(&mut file, "----")?;
                }
            }
        }

        // TODO: pgtest here maybe?
